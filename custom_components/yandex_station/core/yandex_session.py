...
class YandexSession(BasicSession):
    ...
    async def login_token(self, x_token: str) -> bool:
        ...
        # Use clean CookieJar without accumulated tracking cookies to bypass anti-fraud
        clean_jar = CookieJar()
        setattr(clean_jar, "_quote_cookie", False)
        
        temp_session = ClientSession(cookie_jar=clean_jar)
        try:
            payload = {"type": "x-token", "retpath": "https://www.yandex.ru"}
            headers = {"Ya-Consumer-Authorization": f"OAuth {x_token}"}
            r = await temp_session.post(
    ...
    async def refresh_cookies(self) -> bool:
        ...
        # check cookies with probe request
        try:
            r = await self._get("https://yandex.ru/quasar?storage=1")
            resp = await r.json()
            if resp.get("storage", {}).get("user", {}).get("uid"):
                # if cookies valid (uid present) - return without re-login
                _LOGGER.debug("Cookies are valid, uid found")
                return True
        except Exception as e:
            _LOGGER.debug(f"Probe request failed: {e}")

        # refresh cookies
        if not self.x_token:
            _LOGGER.error("Cannot refresh cookies: no x_token")
            return False
    ...
    async def _ensure_csrf_token(self):
        """Lazy load CSRF token when needed."""
    ...
    async def request(self, method: str, url: str, retry: int = 2, **kwargs):
        ...
        if r.status == 200:
            ...
            # 403 - try without CSRF for non-GET, or drop token
            if method != "get" and self.csrf_token:
                _LOGGER.debug("403 on non-GET with CSRF, resetting and retrying")
